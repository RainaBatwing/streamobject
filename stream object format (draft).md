...
header = {
  ...
  audience:
    ...
    "b58string of blake2s index hash (2 byte digest length)": [
...
CipherPermits are stored in a hash table, keyed by a short 2 byte hash of
...
shared key in plaintext. Author does not know if this is a terrible
...
include hundreds if not thousands of permits, and needs to run on devices much
slower than the author's laptop.

It is hoped that the 2 byte index hash doesn't provide enough information to
